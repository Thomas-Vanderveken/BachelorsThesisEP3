...
def extract_info(text, l_temp):
    ...
    for i, part in enumerate(parts):
        if cik_pattern.match(part):
            ...
            numbers_after_cik = re.findall(r'\b\d+(?:[a-zA-Z]{1,3})?\b', " ".join(parts[i+1:]))
    ...
    if not temporary_number_sequence:
        value1 = l_temp[-3] if len(l_temp) > 2 else 0
        value2 = l_temp[-2] if len(l_temp) > 1 else 0
        value3 = l_temp[-1] if len(l_temp) > 0 else 0

        # Create the formatted string
        temporary_number_sequence = f"{value1}, {value2}, {value3}"        
...
def write_to_csv(data, filename='results.csv'):
    ...
    with open(filename, mode='a', newline='', encoding='utf-8') as file:
        ...
        if file.tell() == 0:
            writer.writerow(['file_number', 'Company Name', 'Ticker', 'CIK', 'Value',  'Amount', 'Temporary Number Sequence'])
        # Write the data
        writer.writerow(data)

# Function to process each line and write results to CSV
def process_lines(lines, list_temp_numbers, file_number):
    c = 0
    ...
    for line in lines:
        ...
        write_to_csv([file_number, company_name, ticker, cik, number_sequence.split(' ')[0], number_sequence.split(' ')[1], temporary_number_sequence])
        c+=1

# Sample multiline text block

def get_values(text, file_number):
    list_temporary_numbers = extract_number(text)
    
    lines = text.strip().split('\n')

    # Process each line and write results to CSV
    process_lines(lines, list_temporary_numbers, file_number)
